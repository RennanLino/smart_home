...
class Camera(BaseDevice):
    name_pt = "Camera"
    state = EnumDescriptor(CameraState)
    states = CameraState
    ...
    def __str__(self):
        return (
            f"{self.name_pt}: '{self.name}' [{self.state}] Tempo total ligado: {self.total_time}"
        )

    @property
    def total_time(self):
        if self.state != CameraState.OFF and self.__turned_on_at:
